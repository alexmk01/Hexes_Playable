...
namespace Game.Rendering
{
    public sealed class HexGridRenderer : IInitializable, IDisposable
    {
        ...
        private void HighlightHex(Vector2Int hex)
        {
            ...
            {
                if (hexRenderers.TryGetValue(currentHighlightedHex, out HexComponent currentHexRenderer))
                {
                    currentHexRenderer.Color = DefaultHexColor;
                }

                if (hexRenderers.TryGetValue(hex, out HexComponent newHexRenderer))
                {
                    newHexRenderer.Color = HexHighlightColor;
                }

                currentHighlightedHex = hex;
                HighlightedHexChanged?.Invoke(this, currentHexRenderer, newHexRenderer);
            }
        }
        ...
        private void OnDragging(GameObject obj, Vector3 dragPosition, Vector3 targetPosition)
        {
            ...
            if (hexGrid.CanBeAddedTo(targetHex)) HighlightHex(targetHex);
            else ResetHexHighlight();
        }
        ...
        (
            ...
            DragAndDropHandlerComponent dragAndDropHandler
        )
        {
        ...
        public void Initialize()
        {
            ...
            {
                HexComponent hexRenderer = Object.Instantiate(gameConfig.GameFieldHexPrefab).GetComponent<HexComponent>();
                if (DefaultHexColor.a == 0f) DefaultHexColor = hexRenderer.Color;
                Vector3 worldPos = hexGrid.HexToWorld(hex);
                float hexExtents = hexRenderer.Bounds.extents.y;
                if (groundHexExtents < hexExtents) groundHexExtents = hexExtents;
            ...
            {
                GameObject ground = Object.Instantiate(gameConfig.GroundPrefab);
                ground.transform.position = new Vector3(0f, -groundHexExtents * 2f, 0f);
            }
