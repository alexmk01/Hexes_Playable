            ...
            for (int i = 0; i < playerStacks.Count; i++)
            {
                ...
                for (int j = 0; j < nonPlayerStacks.Count; j++)
                {
                    ...
                    if (nonPlayerTopHex.HexType == topHexType)
                    {
                        ...
                        {
                            if (!hexGrid.IsValidHex(neighbourHex) || hexGrid.IsBusyHex(neighbourHex))
                            {
                                continue;
                            }

                            if (!viewTransform.TryTransformScreenToLocalPosition(camera, playerTopHex.transform.position, out hexStackPosition))
                            {
                                continue;
                            }

                            if (!viewTransform.TryTransformScreenToLocalPosition(camera, hexGrid.HexToWorld(neighbourHex), out hexCellPosition))
                            {
                                continue;
                            }
        ...
        private IEnumerator ViewUpdateCoroutine()
        {
            ...
            while (true)
            {
                if (viewUnhideTime >= 0f)
                {
                    ...
                    {
                        if (TryFindTutorialPointerPoints(out Vector2 hexStackPosition, out Vector2 hexCellPosition))
                        {
