...
namespace Game.UI
{
    public sealed class GameEndScreenPresenter : IInitializable, IDisposable
    {
        ...
        {
            if (result == GameResult.Win) winScreenView.Show();
            else loseScreenView.Show();
        }

        private void OnExitButtonClicked() => playworksService.GoToGameInstallation();
        
        public GameEndScreenPresenter
        (
            GameplayManager gameplayManager,
            PlayworksService playworksService,
            [Inject(Id = WinScreenKey)] GameEndScreenViewComponent winScreenView,
            [Inject(Id = LoseScreenKey)] GameEndScreenViewComponent loseScreenView
        )
        {
