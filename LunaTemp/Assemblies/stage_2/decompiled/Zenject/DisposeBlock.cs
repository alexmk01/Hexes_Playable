...
namespace Zenject
{
	...
	public class DisposeBlock : IDisposable
	{
		...
		private static void OnDespawned(DisposeBlock that)
		{
			...
			{
				for (int j = that._disposables.Count - 1; j >= 0; j--)
				{
					that._disposables[j].Dispose();
				}
		...
		private void StoreSpawnedObject<T>(T obj, IDespawnableMemoryPool<T> pool)
		{
			...
			SpawnedObjectPoolPair spawnedObjectPoolPair = default(SpawnedObjectPoolPair);
			spawnedObjectPoolPair.Pool = pool;
			spawnedObjectPoolPair.Object = obj;
			SpawnedObjectPoolPair pair = spawnedObjectPoolPair;
