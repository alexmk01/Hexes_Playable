	...
	public class MemoryPoolBase<TContract> : IValidatable, IMemoryPool, IDisposable
	{
		...
		public void Resize(int desiredPoolSize)
		{
			if (_inactiveItems.Count != desiredPoolSize)
			{
				...
				{
					throw new PoolExceededFixedSizeException("Pool factory '{0}' attempted resize but pool set to fixed size of '{1}'!".Fmt(GetType(), _inactiveItems.Count));
				}
		...
		private void ExpandPool()
		{
			...
			case PoolExpandMethods.Disabled:
				throw new PoolExceededFixedSizeException("Pool factory '{0}' exceeded its fixed size of '{1}'!".Fmt(GetType(), _inactiveItems.Count));
