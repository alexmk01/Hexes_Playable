	...
	public sealed class HexDatabase
	{
		...
		public bool TryGetHexData(string hexName, out HexData hexData)
		{
			if (!string.IsNullOrEmpty(hexName))
			{
				...
				{
					if (string.Equals(hexesData[i].Name, hexName, StringComparison.OrdinalIgnoreCase))
					{
						hexData = hexesData[i];
						return true;
					}
		...
		public int GetRandomHexType()
		{
			return UnityEngine.Random.Range(0, hexesData.Length);
		}
	}
}
