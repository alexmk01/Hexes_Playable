...
namespace Game.Animation
{
	public sealed class HexAnimationService : IInitializable, IDisposable
	{
		...
		{
			hexAnimationDuration = duration / (float)hexCount;
		...
		private void OnAddingNewPlayerStack(GameplayManager manager, HexStackComponent playerStack, float duration)
		{
			if (!(duration <= 0f))
			{
				...
				animation.Append(stackTransform.DOScale(targetScale, duration - punchScaleDuration).SetEase(Ease.InOutQuad));
				animation.Append(stackTransform.DOPunchScale(Vector3.one * 0.2f, punchScaleDuration, 3));
			}
		}
		...
		private void OnMergingHexStack(GameplayManager manager, HexStackComponent sourceStack, HexStackComponent targetStack, int hexCount, float duration)
		{
			...
			Quaternion targetRotation = sourceTopHex.transform.rotation * Quaternion.AngleAxis(180f, rotationAxis);
			float hexAnimationDuration;
			Sequence animation = CreateStackAnimation(targetStack, hexCount, duration, out hexAnimationDuration);
			...
			foreach (HexComponent hex in sourceStack.GetTopHexes(hexCount))
			{
				...
				animation.Insert(startTime, hexTransform.DOJump(targetHexPosition, jumpHeight, 1, hexAnimationDuration).SetEase(Ease.InOutQuad));
				animation.Insert(startTime, hexTransform.DORotateQuaternion(targetRotation, hexAnimationDuration).SetEase(Ease.InOutQuad));
				animation.AppendCallback(delegate
				{
					hexTransform.rotation = initialRotation;
				});
		...
		private void OnDestroyingStackHexes(GameplayManager manager, HexStackComponent stack, int hexCount, float duration)
		{
			...
			float hexAnimationDuration;
			Sequence animation = CreateStackAnimation(stack, hexCount, duration, out hexAnimationDuration);
			foreach (HexComponent hex in stack.GetTopHexes(hexCount))
			{
				animation.Append(hex.transform.DOScale(Vector3.zero, hexAnimationDuration).SetEase(Ease.InOutQuad).OnComplete(delegate
				{
					hex.Renderer.enabled = false;
				}));
			}
		}
		...
		private void OnHighlightedHexChanged(HexGridRenderer renderer, HexComponent lastHex, HexComponent newHex)
		{
			if (lastHex != null)
			{
				DOVirtual.Color(renderer.HexHighlightColor, renderer.DefaultHexColor, gameConfig.HexHighlightDuration, delegate(Color color)
				{
					lastHex.Color = color;
				}).SetEase(Ease.InOutQuad).SetLink(lastHex.gameObject);
			}
			if (newHex != null)
			{
				DOVirtual.Color(renderer.DefaultHexColor, renderer.HexHighlightColor, gameConfig.HexHighlightDuration, delegate(Color color)
				{
					newHex.Color = color;
				}).SetEase(Ease.InOutQuad).SetLink(newHex.gameObject);
			}
		}
